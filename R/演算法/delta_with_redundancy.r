delta_with_redundancy=function(data_A,folder,result_D,test_data,s,Discrete,criteria,option,test_M_data,beta_value=0,seed){
	...
	for(i in 1:nrow(result_D)){
		...
		msg=tryCatch({
			op=as.data.frame(read.csv(file=paste0("C:/R/Result/",folder,"/",filename,"_multi_criterion_",(ncol(ary)+1),".csv"),1))
			for(h in 1:nrow(op)){	
		...
		multi_criterion=""
			a2=parSapply(cl,(which(colnames(ary[ncol(ary)])==colnames(a))+1):(ncol(a)),function(k,Discrete,a,temp_matrix,ste,ary,criteria,multi_criterion_pre,multi_criterion,result_D,position1,nnn1,data_A,cl,test_M_data,option,Discrete_data){
				...
				t2=FCM_C_AutoSelect(t1,Discrete,seed)			
				...
				interior_cor=abs(interior(t1,test_M_data,criteria))
			...
			},Discrete,a,temp_matrix,ste,ary,criteria,multi_criterion_pre,multi_criterion,result_D,position1,nnn1,data_A,cl,test_M_data,option,Discrete_data)
			if(i ==1){
			...
			a2=parSapply(cl,(which(colnames(ary[ncol(ary)])==colnames(a))+1):(ncol(a)),function(k,Discrete,msg,a,temp_matrix,ste,ary,criteria,multi_criterion_pre,multi_criterion,result_D,position1,nnn1,data_A,cl,test_M_data,option,Discrete_data){
			...
			},Discrete,msg,a,temp_matrix,ste,ary,criteria,multi_criterion_pre,multi_criterion,result_D,position1,nnn1,data_A,cl,test_M_data,option,Discrete_data)
			if(i ==1){
				aa2_multi_criterion=data.frame(matrix(unlist(a2[1,]),ncol=length(a2[1,])))
				aa2_Discrete_data=data.frame(matrix(unlist(a2[2,]),ncol=length(a2[2,])))
				aa2_temp_matrix=data.frame(matrix(unlist(a2[3,]),nrow=length(a2[3,]),byrow=T))
	...
	write.csv(aaa2_multi_criterion,file=paste0("C:/R/Result/",folder,"/",filename,"_multi_criterion_",(ncol(ary)+1),".csv"))
	return(ans)
}