delta=function(data_A,folder,result_D,test_data,s,Discrete,criteria,option,test_M_data,beta_value=0,seed){
		...
		multi_criterion=""
			...
			a2=parSapply(cl,(which(colnames(ary[ncol(ary)])==colnames(a))+1):(ncol(a)),function(k,Discrete,msg,a,temp_matrix,ste,ary,criteria,multi_criterion_pre,multi_criterion,result_D,position1,nnn1,data_A,cl,test_M_data,option,Discrete_data){
				...
				if(length(msg[which(rownames(msg)==option),which(colnames(msg)==paste0(nnn1,feature))])==0){
					...
					t_result=criteria(data_A,t2,seed)		#進行剛剛離散值t，再進行評估法的計算
					initial=as.data.frame(t_result[1])
